...
get_ipython().system('pip install -q --upgrade numpy')
get_ipython().system('pip install -q --upgrade tensorflow tensorflow-model-optimization')
...
KD_PHASES = [
    ("kd_phase1_all_frozen",    120, "fer13",  3e-4, 15,  4, 0.9, 50.0),  # Heavy teacher feature matching
    ("kd_phase2_top60_frozen",   60,   "fer13",  3e-5, 15,  4, 0.8, 50.0),  
    ("kd_phase3_top30_frozen",   30,   "fer13",  3e-6, 30,  5, 0.7, 10.0),  # Loosen feature matching
    ("kd_phase4_rafdb",          30,   "rafdb",  3e-6, 75,  5, 0.7, 10.0),  
    ("kd_phase5_afd",            30,   "afd",    5e-6, 120, 5, 0.9, 0.0),   # Cut the cord! 90% Focal Loss.
]

print("Starting Knowledge Distillation")
student_histories = []

for phase_idx, (phase_name, freeze_spec, dataset_name, lr, max_epochs, patience, alpha, beta) in enumerate(KD_PHASES):
    print(f"\n{'='*60}")
   ...
   # Since the model is flat, the backbone is everything EXCEPT the last 5 layers we added
    ...
    distiller.beta = beta    # update feature matching dynamically
    distiller.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=lr, epsilon=1e-7),
